...
cd $4 && echo build-log-separator && mvn -DskipTests package && echo build-log-separator && \
cd .. && java -cp "${4}"/target/"${4}"-1.0-SNAPSHOT.jar:"${5}"-1.0-SNAPSHOT-solution-tests.jar TestMain $6
#cd $4 && echo build-log-separator && gradle assemble && echo build-log-separator && \
#cd .. && java -cp "${4}"/build/libs/"${4}"-1.0-SNAPSHOT.jar:"${5}"-1.0-SNAPSHOT-solution-tests.jar TestMain $6
