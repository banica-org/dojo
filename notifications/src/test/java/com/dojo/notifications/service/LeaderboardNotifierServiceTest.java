...
public class LeaderboardNotifierServiceTest {
    ...
    private final Leaderboard NEW_LEADERBOARD = new Leaderboard(Arrays.asList(SECOND_PARTICIPANT, FIRST_PARTICIPANT,THIRD_PARTICIPANT));
    ...
    public void notifyPersonalChangesTest() {
        //Arrange
        SELECT_REQUEST.setEVENT_TYPE("POSITION_CHANGES");
        ...
        when(flinkTableService.executeSingleQuery(any(),eq("NewLeaderboard"), any())).thenReturn(NEW_LEADERBOARD.getParticipants());
        when(flinkTableService.executeSingleQuery(any(),eq("OldLeaderboard"), any())).thenReturn(OLD_LEADERBOARD.getParticipants());
        when(leaderboardService.determineEventType(any(),any(),eq(EventType.POSITION_CHANGES))).thenReturn(true);
        ...
        verify(leaderboardService,times(1)).determineEventType(any(),any(),eq(EventType.POSITION_CHANGES));
    ...
    public void notifyCommonChangesTest() {
        //Arrange
        SELECT_REQUEST.setEVENT_TYPE("SCORE_CHANGES");
        List<SelectRequest> requests = new ArrayList<>(Collections.singletonList(SELECT_REQUEST));

        when(selectRequestService.getRequests()).thenReturn(requests);
        when(flinkTableService.executeSingleQuery(any(),eq("NewLeaderboard"), any())).thenReturn(NEW_LEADERBOARD.getParticipants());
        when(flinkTableService.executeSingleQuery(any(),eq("OldLeaderboard"), any())).thenReturn(OLD_LEADERBOARD.getParticipants());
        when(leaderboardService.determineEventType(any(),any(),eq(EventType.SCORE_CHANGES))).thenReturn(true);
        when(contest.getPersonalNotifiers()).thenReturn(Collections.singleton(NotifierType.EMAIL));
        when(contest.getCommonNotificationsLevel()).thenReturn(leaderBoardNotificationsType);
        ...
        verify(flinkTableService, times(1)).executeSingleQuery(any(),eq("NewLeaderboard"), any());
        verify(flinkTableService, times(1)).executeSingleQuery(any(),eq("OldLeaderboard"), any());
        verify(leaderboardService,times(1)).determineEventType(any(),any(),eq(EventType.SCORE_CHANGES));
        verify(contest, times(2)).getContestId();
        verify(contest, times(1)).getCommonNotificationsLevel();
        verify(notificationService, times(1)).notify(any(), any());
    }
